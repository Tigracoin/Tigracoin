...
const string strMessageMagic = "TigraCoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    else if (nBestHeight < 40000) // 250 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 2.5;
    }

    else if (nBestHeight < 60000) // 500 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 5;
	}

    else if (nBestHeight < 80000) // 750 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 7.5;
    }

    else if (nBestHeight < 120000) // 1000 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 10;
    }

    else if (nBestHeight < 140000) // 1250 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 12.5;
    }

    else if (nBestHeight < 160000) // 1500 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 15;
    }

    else if (nBestHeight < 180000) // 1000 %
    {
	nSubsidy = nCoinAge * nRewardCoinYear / 365 * 10;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TigraCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "What does't kill you makes you stronger, except for bears, bear will kill.";
        CTransaction txNew;
        txNew.nTime = 1523920164;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1523920164;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1158542 : 1045568;
                
        assert(block.hashMerkleRoot == uint256("0xeadab1e1424b8e5654c692153e0535048b23fde6646e84e1d1de802b7031b096"));
...
unsigned char pchMessageStart[4] = { 0x1b, 0xb1, 0xd3, 0xd9 };
